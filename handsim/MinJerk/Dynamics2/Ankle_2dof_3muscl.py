...
class Ankle_2dof(nn.Module):
    def __init__(self, device, EMG_Channel_Count, Left, Dyanmic_Lr, EMG_mat_Lr, NN_ratio, \
        ...
        self.m1 = Muscle(1000, 20000, 0.03, 0.006, [0.0, 0.02])
        self.m2 = Muscle(1000, 20000, 0.03, 0.006, [0.0173, -0.01])
        self.m3 = Muscle(1000, 20000, 0.03, 0.006, [-0.0173, -0.01])
         
        self.muscles = [self.m1, self.m2, self.m3]
        ...
        self.EMG_to_Activation_Mat_p = nn.Parameter(torch.ones((self.EMG_Channel_Count, self.muscle_num), dtype=torch.float, device=self.device)/self.EMG_mat_Lr/self.EMG_Channel_Count)
    ...
    def forward(self, SS, EMG, dt):
        self.EMG_to_Activation_Mat = torch.abs(self.EMG_to_Activation_Mat_p)
    ...
    def lock_EMG_mat(self, switch=True):
        self.EMG_to_Activation_Mat_p.requires_grad = not switch
        
    def lock_TA_K(self, switch=True):
        self.Joint.K1s[0].requires_grad = not switch
