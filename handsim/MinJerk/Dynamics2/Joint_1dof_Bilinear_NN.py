    ...
    def forward(self, SS, Alphas, dt=0.0166667):
        ...
        if(self.speed_mode == False):
            ...
            M = torch.hstack([torch.dstack([A*dt, B*dt, torch.zeros((batch_size, 2, 2), dtype=torch.float, device=self.device)]),
                              torch.dstack([torch.zeros((batch_size, 2, 4), dtype=torch.float, device=self.device), torch.tensor(
...
model = Joint_1dof(device, [m1, m2], [0.004], Lr_scale=5, NN_ratio=1, speed_mode=False)
# model = Joint_2dof(device, [m1, m2, m3, m4], [0.004, 0.008], Lr_scale=5, NN_ratio=0.0, speed_mode=True)
model.eval()
X = torch.FloatTensor([[1, 0], [0, 0], [1, 0]])
a1 = torch.FloatTensor([[0, 0], [0.2, 0], [0, 1]])
# For time benchmark
start = time.time()
for i in range(60):
    w, X = model(X, a1, 0.0166667)
    # print(X)
